...
inline Thread::Thread(void (* entry)(Tn ...), Tn ... an)/* inicialização de _link */
{
    ...
    _context = new CPU::Context(entry, (char *) an ... ); // criação de contexto
    thread_counter++; // incremento do contador de threads
    List::List_Element_rank(Ready_Queue) // Inicilização da lista de threads
}
